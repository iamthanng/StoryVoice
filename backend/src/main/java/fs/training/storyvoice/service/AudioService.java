...
public class AudioService {
     ...
     *   4. Nếu CHƯA CÓ: Gọi ElevenLabs API -> Lưu MP3 đĩa cứng -> Lưu DB -> Trả về DTO
     */
    @Transactional
    public ChapterResponse generateOrGetTtsAudio(Long chapterId, String voiceId, UserPrincipal currentUser) {
        ...
        // 3. Nếu chưa có cache -> Gọi ElevenLabs TTS API
        byte[] audioBytes = elevenLabsTtsService.generateSpeech(chapter.getContent(), voiceId);
    ...
    public ResponseEntity<ResourceRegion> streamAudioResource(Long audioId, String rangeHeader) {
        AudioFile audioFile = audioFileRepository.findById(audioId)
                .orElseThrow(() -> new RuntimeException("Không tìm thấy file audio với ID: " + audioId));
        ...
        if (!resource.exists()) {
            throw new RuntimeException("Tập tin âm thanh không tồn tại trên ổ đĩa server");
        }
        ...
        try {
            ...
            throw new RuntimeException("Không thể stream file audio", ex);
        }
    }
}
